...
public class StockPerformanceService {
    ...
    public List<StockPerformanceDTO> stockPerformanceForDateAndTimeframeAndFilters(StockTimeframe timeFrame, LocalDate date, Boolean xtb, Double cfdMargin) {
        String query = queryStringFrom(timeFrame, xtb);

        Query nativeQuery = entityManager.createNativeQuery(query, tableClassFrom(timeFrame));
    ...
    private static String queryStringFrom(StockTimeframe timeFrame, boolean xtb) {
    ...
    private Pair<LocalDate, LocalDate> intervalFor(LocalDate date, StockTimeframe timeFrame) {
        return switch (timeFrame) {
            case WEEKLY -> Pair.of(date.with(TemporalAdjusters.previousOrSame(DayOfWeek.MONDAY)), date.with(TemporalAdjusters.nextOrSame(DayOfWeek.FRIDAY)));
