...
public class PriceMilestoneCache {
    ...
    private boolean priceWithinMilestone(Stock s, HighLowForPeriod highLowForPeriod, PriceMilestone priceMilestone) {
        return switch (priceMilestone) {
            case NEW_52W_HIGH, NEW_ALL_TIME_HIGH, NEW_4W_HIGH -> s.getWeeklyHigh() == highLowForPeriod.getHigh(); // todo: fix logic to use previous weeks high-lows
            case NEW_52W_LOW, NEW_4W_LOW, NEW_ALL_TIME_LOW -> s.getWeeklyLow() == highLowForPeriod.getLow(); // todo: fix logic to use previous weeks high-lows
