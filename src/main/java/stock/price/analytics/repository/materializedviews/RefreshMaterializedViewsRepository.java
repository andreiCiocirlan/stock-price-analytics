...
public interface RefreshMaterializedViewsRepository extends JpaRepository<AbstractPriceOHLC, Long> {
    ...
    @Modifying
    @Query(value = "REFRESH MATERIALIZED VIEW quarterly_prices_performance_view", nativeQuery = true)
    void refreshQuarterlyPerformanceHeatmapPrices();
    ...
    @Modifying
    @Query(value = "REFRESH MATERIALIZED VIEW daily_prices_json_view", nativeQuery = true)
    void refreshDailyJSONPricesPrices();
