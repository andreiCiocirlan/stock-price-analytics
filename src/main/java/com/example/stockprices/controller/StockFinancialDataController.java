...
import com.example.stockprices.client.FinnhubClient;
import com.example.stockprices.service.StockFinancialDataService;
...
import static com.example.stockprices.util.FileUtils.writeToFile;
...
public class StockFinancialDataController {
    ...
    public void financialDataToJSONFile() throws IOException, InterruptedException {
        File file = ResourceUtils.getFile(CLASSPATH_TICKERS_XTB_TXT);
        ...
        for (String ticker : FileUtils.readTickersXTB()) {
            ...
            writeToFile(Collections.singletonList(responseEntity.getBody()), outputFile);
        }
    }
